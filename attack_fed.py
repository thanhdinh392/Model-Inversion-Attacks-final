...
if __name__ == "__main__":
    ...
    target_amplification = (
        PowerAmplification(args.num_classes, 1 / 30).train(False).to(args.device)
    )
    ...
    if args.type_fed == "client":
        ...
        if args.iid:
            if args.noise:
                ...
                if args.sigma == 0.001:  # min
                    client_path = f"./noise_gauss_min/{args.dataset}/log_main_iid/log_client"
                if args.sigma == 0.01:  # max
                    client_path = f"./noise_gauss_max/{args.dataset}/log_main_iid/log_client"
            else:
        ...
        for epoch_file in sorted_files:
            ...
            if args.iid:
                ...
                ldr_train_li = [DataLoader(DatasetSplit(dataset_train, dict_users[idx]), batch_size=args.local_bs, shuffle=True) for idx in range(args.num_users)]
            idx = 0
            for pkl_file in sorted(os.listdir(os.path.join(client_path, epoch_file))):  # client 0, 1, 2, 3, 4
                print(pkl_file)
                ...
                with torch.no_grad():
                    ...
                    pixelloss = 0
                    id_img = 0
                    for i, (im, label) in enumerate(tqdm(ldr_train_li[idx], desc=f"priv")):
    ...
    # global
        ...
        if args.iid:
            if args.noise:
                ...
                if args.sigma == 0.001:  # min
                    global_path = f"./noise_gauss_min/{args.dataset}/log_main_iid/log_global_model"
                if args.sigma == 0.01:  # max
                    global_path = f"./noise_gauss_max/{args.dataset}/log_main_iid/log_global_model"
            else:
